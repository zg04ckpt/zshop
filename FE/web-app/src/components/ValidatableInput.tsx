...
import '../styles/components/ValidatableInput.css';
...
export const ValidatableInput = <T,>(prop: InputProp<T>) => {
    ...
    return (
        <div className="validation-input" style={{
            ...
            { prop.isMultiLine && <textarea ref={prop.ref} value={value} onFocus={() => setFocus(true)} onChange={e => setValue(e.target.value)} className={`input ${error? 'invalid':''}`}></textarea> }
            { !prop.isMultiLine && <input ref={prop.ref} value={value} onFocus={() => setFocus(true)} onChange={e => setValue(e.target.value)} className={`input ${error? 'invalid':''}`} type={prop.type}/> }
